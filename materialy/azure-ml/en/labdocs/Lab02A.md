...
4. On the left side of the designer, expand the **Data** section, and drag the **diabetes_dataset** data asset you created in the previous exercise onto the canvas.
5. Select the **diabetes_dataset** module on the canvas, and view its settings. Then on the **outputs** tab, click the **Visualize** icon (which looks liks a column chart).
