...
## Task 3: Use the Azure Machine Learning SDK v2 to Access the Datastore

In this task, you'll use the Azure Machine Learning SDK v2 to register a data asset from local files, and use it as an input to a training job.
...
3. In the JupyterLab file browser, open the **MachineLearningCourse/materialy/azure-ml/en** folder (cloning or pulling the repository first if you haven't already - see [Lab 1B](Lab01B.md)), then open the **04A - Working with Datastores.ipynb** notebook. Confirm that it uses the **Python 3.10 - SDK v2** kernel, then read the notes in the notebook, running each code cell in turn.

> **Note**: If you intend to continue straight to the [next exercise](Lab04B.md), leave your compute instance running. If you're taking a break, you might want to close all JupyterLab tabs and **Stop** your compute instance to avoid incurring unnecessary costs.
